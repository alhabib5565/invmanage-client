...
import { useGetAllSalesRetrunsQuery } from "@/redux/api/admin/salesRetrun.api";
import { TSalesRetrun } from "./salesReturn.type";

const SalesReturnList = () => {
  const { data } = useGetAllSalesRetrunsQuery({});
  console.log({ data });
  return (
      ...
      <div className="p-6 border rounded-[16px] space-y-4 bg-white">
        ...
        <div className="relative h-fit max-h-[500px] overflow-y-scroll">
          <Table className="border-b">
            ...
            <TableBody>
              {data?.data.map((purchase: TSalesRetrun) => (
